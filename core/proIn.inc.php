...
function addProIn(){
	...
	if($res&&$pid){
		...
		$mes="<p>添加成功!</p><a href='addPro_in.php' target='mainFrame'>继续添加</a>|<a href='Pro_in.php' target='mainFrame'>查看商品列表</a>";
	}else{
		foreach($uploadFiles as $uploadFile){
		...
		$mes="<p>添加失败!</p><a href='addPro_in.php' target='mainFrame'>重新添加</a>";
...
function editProIn($id){
	...
	$where="id={$id}";
	$res=update("imooc_pro_in",$arr,$where);
	$pid=$id;
	if($res&&$pid){
		...
		$mes="<p>编辑成功!</p><a href='Pro_in.php' target='mainFrame'>查看商品列表</a>";
	}else{
	if(is_array($uploadFiles)&&$uploadFiles){
		foreach($uploadFiles as $uploadFile){
			if(file_exists("../image_800/".$uploadFile['name'])){
		...
		$mes="<p>编辑失败!</p><a href='Pro_in.php' target='mainFrame'>重新编辑</a>";
		
	}
	return $mes;
}

function delProIn($id){
	$where="id=$id";
	$res=delete("imooc_pro_in",$where);
	...
	if($res&&$res1){
		$mes="删除成功!<br/><a href='Pro_in.php' target='mainFrame'>查看商品列表</a>";
	}else{
		$mes="删除失败!<br/><a href='Pro_in.php' target='mainFrame'>重新删除</a>";
	}
	return $mes;
}
...
// 	$sql="select i.id,i.pSn,i.pName,i.iPrice,i.size,i.pNum,i.sumPrice,i.pubTime,i.firm,i.operator,c.cName from imooc_pro_in as i join imooc_cate c on i.cId=c.id";
// 	$rows=fetchAll($sql);
// 	return $rows;
// }
 ...
 * @return array
 */
function getProInById($id){
		$sql="select i.id,i.NO,i.pSn,i.pName,i.iPrice,i.size,i.pNum,i.sumPrice,i.pubTime,i.firm,i.operator,c.cName from imooc_pro_in as i join imooc_cate c on i.cId=c.id where i.id={$id}";
		$row=fetchOne($sql);
		return $row;
}
// /**
